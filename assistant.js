...
function buildSystemPrompt(userName) {
  const addr = userName ? `${userName}さん` : 'あなた';
  return `Python3エンジニア認定基礎試験を学習している人の専属アシスタントです。
ペアプログラマーとして対等に対話してください。
...
【相手への呼びかけ】
${userName ? `相手は ${userName}さん。${addr}と呼びかけてください。` : '相手の名前は不明。「あなた」と呼びかけてください。'}

【Python基礎試験で問われる典型的な誤答癖（参考にしてよい7分類）】
...
【教える際のヒント】
- これらの癖に該当する論点では、相手が「ありがちに陥る誤答パターン」を先に提示し、なぜそう間違えるかを言語化してから正解へ導く
- 「間違えた経験を通して記憶」と「正しい思考プロセスのトレース」を意識する
- 日常の比喩（カフェ・工場・荷物など）を使うと理解が深まる
...
// ========== 統計コンテキスト生成（誤答癖を埋め込む） ==========
function buildContextSummary(userName) {
  if (typeof QUESTIONS === 'undefined' || typeof getMeta !== 'function') {
    return '';
  }
  const subject = userName ? `${userName}さん` : 'この学習者';
  ...
  return `\n【現在の${subject}の誤答癖プロファイル（ドリル統計より）】\n${lines.join('\n')}\n`;
}
...
async function callGemini(messages, settings) {
  ...
  const systemPrompt = settings.useContext
    ? basePrompt + buildContextSummary(settings.userName)
    : basePrompt;
